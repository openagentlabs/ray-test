...
from iam_service.core import cloudwatch_logging_defaults_generated as tfdefaults
from iam_service.observability.correlation import get_correlation_id
...
class _CloudWatchLogRecordExporter(LogRecordExporter):
    ...
    def export(self, batch: Sequence[ReadableLogRecord]) -> LogRecordExportResult:
        ...
        except Exception:  # noqa: S110 — CloudWatch failures must not surface or recurse into logging
            # Never block or fail the app when CloudWatch is unavailable.
            pass
        return LogRecordExportResult.SUCCESS
    ...
    def _put_with_retry(self, events: list[dict[str, object]]) -> None:
        ...
        try:
            ...
            if code == "InvalidSequenceTokenException":
                self._sequence_token = err.response["Error"].get("expectedSequenceToken")
                kwargs["sequenceToken"] = self._sequence_token
                response = self._client.put_log_events(**kwargs)
                self._sequence_token = response.get("nextSequenceToken")
                return
