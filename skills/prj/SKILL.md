...
description: >-
  ...
  .cursor/rules/constants/constants.mdc one constant at a time with validation. Use when
...
Path: **`skills/prj/`** (entry router: **`.cursor/skills/ray-test/SKILL.md`**)

## What · When · How

**What:** Guided **project init** — read **Group 1 — Project (`PRJ_*`)** from **`.cursor/rules/constants/constants.mdc`**, collect new values **one constant per turn**, validate format/constraints, sync derived literals, write back to **`constants.mdc`**.
...
| **[workflow-reference.md](../_shared/workflow-reference.md)** | Unclear on **`Jmp:`** / **`ASK`** semantics |
...
1. **Single source** — Read and write **only** **`.cursor/rules/constants/constants.mdc`** for catalog values; do not duplicate literals in other rules during this skill.
...
| Topic | Path |
|-------|------|
| Constants catalog | **`.cursor/rules/constants/constants.mdc`** |
...
| Workflow DSL | **[workflow-reference.md](../_shared/workflow-reference.md)** |
