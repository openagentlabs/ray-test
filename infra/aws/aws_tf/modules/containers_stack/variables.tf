...
variable "fsx_lustre_enabled" {
  description = "When true, FSx Lustre shared-lustre PVCs are available for EC2 workloads."
  type        = bool
  default     = false
  nullable    = false
}
...
variable "lustre_volume_name" {
  description = "Kubernetes PVC name for shared Lustre (shared-lustre)."
  type        = string
  default     = ""
  nullable    = false
}
...
variable "lustre_mount_path" {
  description = "Container mount path for shared Lustre (/mnt/lustre)."
  type        = string
  default     = "/mnt/lustre"
  nullable    = false
}
...
variable "s3_shared_files_enabled" {
  description = "When true, shared-s3-files PVCs are available for EC2 workloads."
  type        = bool
  default     = false
  nullable    = false
}
...
variable "s3_shared_volume_name" {
  description = "Kubernetes PVC name for Mountpoint S3 (shared-s3-files)."
  type        = string
  default     = ""
  nullable    = false
}
...
variable "s3_shared_mount_path" {
  description = "Container mount path for shared S3 files (/mnt/s3-files)."
  type        = string
...
variable "ray_node_pool_label_key" {
  description = "Node label key for scheduling manager-web on the Ray EC2 pool."
  type        = string
...
variable "ray_node_pool_label_value" {
  description = "Node label value for scheduling manager-web on the Ray EC2 pool."
  type        = string
  default     = "ray"
  nullable    = false
}
