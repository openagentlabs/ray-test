###############################################################################
# IAM role for general.ai.agent.svc — Bedrock Runtime (Anthropic Claude Sonnet family)
###############################################################################
...
variable "bedrock_invoke_resource_arns" {
  description = <<-EOT
    ...
    Built from root variables `general_ai_agent_bedrock_foundation_model_id` and
    `general_ai_agent_bedrock_inference_profile_id`; must align with general.ai.agent.svc
