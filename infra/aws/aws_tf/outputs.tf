...
output "s3_files_bucket_arn" {
  description = "ARN of the storage.svc platform files S3 bucket (tf_lib template)."
  value       = module.s3_files.bucket_arn
}

output "s3_files_bucket_name" {
  description = "Name of the storage.svc platform files S3 bucket."
  value       = module.s3_files.bucket_name
}

output "s3_files_bucket_regional_domain_name" {
  description = "Regional virtual-hosted-style domain for the platform files bucket."
  value       = module.s3_files.bucket_regional_domain_name
}
...
output "containers_alb_controller_iam_role_arn" {
  description = "IAM role ARN for the AWS Load Balancer Controller when EKS is enabled."
  value       = var.containers_eks_enabled ? module.workloads_infra[0].alb_controller_iam_role_arn : ""
}

output "containers_alb_ingress_class" {
  description = "IngressClass for ALB-backed Ingress resources when EKS is enabled."
  value       = var.containers_eks_enabled ? module.workloads_infra[0].alb_ingress_class : ""
}

output "containers_alb_public_subnet_ids" {
  description = "Public subnet IDs for ALBs when containers_eks_enabled is true."
  value       = var.containers_eks_enabled ? module.workloads_infra[0].alb_public_subnet_ids : []
}

output "containers_alb_security_group_id" {
  description = "Security group ID for internet-facing ALBs when EKS is enabled."
  value       = var.containers_eks_enabled ? module.workloads_infra[0].alb_security_group_id : ""
}

output "containers_bastion_subnet_ids" {
  description = "Private subnet IDs for bastion hosts when EKS is enabled."
  value       = var.containers_eks_enabled ? module.workloads_infra[0].bastion_subnet_ids : []
}

output "containers_bastion_security_group_id" {
  description = "Security group ID for bastion hosts when EKS is enabled."
  value       = var.containers_eks_enabled ? module.workloads_infra[0].bastion_security_group_id : ""
}

output "containers_vpc_cidr_block" {
  description = "CIDR block of the platform VPC when EKS is enabled."
  value       = var.containers_eks_enabled ? module.workloads_infra[0].vpc_cidr_block : ""
}

output "containers_vpc_id" {
  description = "Platform VPC ID when EKS is enabled."
  value       = var.containers_eks_enabled ? module.workloads_infra[0].vpc_id : ""
}

output "containers_eks_workloads_security_group_id" {
  description = "Security group for EKS Fargate workload ENIs when EKS is enabled."
  value       = var.containers_eks_enabled ? module.workloads_infra[0].eks_workloads_security_group_id : ""
}
...
output "containers_cluster_security_group_id" {
  description = "EKS-managed primary security group when EKS is enabled."
  value       = var.containers_eks_enabled ? module.workloads_infra[0].cluster_security_group_id : ""
}

output "kuberay_namespace" {
  description = "Kubernetes namespace for KubeRay when kuberay_enabled is true."
  value       = var.containers_eks_enabled && var.kuberay_enabled ? module.workloads_infra[0].kuberay_namespace : ""
}

output "ray_node_group_name" {
  description = "EKS managed node group for Ray compute when KubeRay is enabled."
  value       = var.containers_eks_enabled && var.kuberay_enabled ? module.workloads_infra[0].ray_node_group_name : ""
}

output "ray_dashboard_url" {
  description = "HTTP URL for the Ray dashboard and Jobs API (ALB). Re-check with kubectl if empty immediately after apply."
  value       = var.containers_eks_enabled && var.kuberay_enabled ? module.workloads_infra[0].ray_dashboard_url : ""
}

output "ray_metrics_url" {
  description = "HTTP URL for Ray head Prometheus metrics via ALB."
  value       = var.containers_eks_enabled && var.kuberay_enabled ? module.workloads_infra[0].ray_metrics_url : ""
}

output "ray_alb_hostname" {
  description = "ALB DNS hostname for the Ray dashboard ingress."
  value       = var.containers_eks_enabled && var.kuberay_enabled ? module.workloads_infra[0].ray_alb_hostname : ""
}

output "fsx_lustre_file_system_dns_name" {
  description = "DNS name for mounting the shared FSx for Lustre file system."
  value       = var.containers_eks_enabled && var.fsx_lustre_enabled ? module.workloads_infra[0].fsx_lustre_file_system_dns_name : ""
}

output "fsx_lustre_file_system_id" {
  description = "Identifier of the shared FSx for Lustre file system."
  value       = var.containers_eks_enabled && var.fsx_lustre_enabled ? module.workloads_infra[0].fsx_lustre_file_system_id : ""
}

output "fsx_lustre_mount_name" {
  description = "Lustre mount name for the shared file system."
  value       = var.containers_eks_enabled && var.fsx_lustre_enabled ? module.workloads_infra[0].fsx_lustre_mount_name : ""
}

output "lustre_shared_mount_path" {
  description = "Container mount path for the shared Lustre volume (/mnt/lustre)."
  value       = var.containers_eks_enabled && var.fsx_lustre_enabled ? module.workloads_infra[0].lustre_shared_mount_path : ""
}

output "lustre_shared_volume_name" {
  description = "Kubernetes volume name for the shared Lustre PVC (shared-lustre)."
  value       = var.containers_eks_enabled && var.fsx_lustre_enabled ? module.workloads_infra[0].lustre_shared_volume_name : ""
}

output "s3_shared_files_bucket_arn" {
  description = "ARN of the shared S3 bucket for pod mounts."
  value       = var.s3_shared_files_enabled ? module.s3_shared_files[0].bucket_arn : ""
}

output "s3_shared_files_bucket_name" {
  description = "Name of the shared S3 bucket for pod mounts."
  value       = var.s3_shared_files_enabled ? module.s3_shared_files[0].bucket_name : ""
}

output "s3_shared_mount_path" {
  description = "Container mount path for the shared S3-backed volume (/mnt/s3-files)."
  value       = var.containers_eks_enabled && var.s3_shared_files_enabled ? module.workloads_infra[0].s3_shared_mount_path : ""
}

output "s3_shared_volume_name" {
  description = "Kubernetes volume name for the shared S3 PVC (shared-s3-files)."
  value       = var.containers_eks_enabled && var.s3_shared_files_enabled ? module.workloads_infra[0].s3_shared_volume_name : ""
}
