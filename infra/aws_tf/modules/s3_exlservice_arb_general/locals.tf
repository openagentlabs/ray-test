###############################################################################
# infra/tf_lib/s3 — locals (single place for derived values used across files)
#
...
locals {
  bucket_name = var.bucket_name_override != null ? var.bucket_name_override : lower(replace(
    "${var.solution.name}-${var.purpose}-${var.solution.account_id}",
    "_",
    "-",
  ))
