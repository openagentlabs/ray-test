...
locals {
  ...
  collection_name = substr(
    lower(replace("${var.solution.name}-doc-vec-${var.solution.account_id}", "_", "-")),
    0,
    32,
  )
  attachments_bucket = lower(replace(
    "${var.solution.name}-docstore-attachments-${var.solution.account_id}",
    "_",
    "-",
  ))
}
