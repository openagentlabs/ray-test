...
def _frontend_load_balancer_host(namespace: str) -> str:
    proc = run(
        [
            "kubectl",
            "get",
...
def phase_validate_curl(outputs: dict[str, Any]) -> PhaseResult:
    ...
    lb_host = _frontend_load_balancer_host(namespace)
    if not lb_host:
        return PhaseResult("validate_curl", False, "frontend LoadBalancer hostname not ready")

    curl_errors: list[str] = []
    for label, url in [("frontend /login", f"http://{lb_host}/login"), ("frontend /", f"http://{lb_host}/")]:
    ...
    return PhaseResult(
        ...
        data={"frontend_url": f"http://{lb_host}/login"},
    )
    ...
    while time.monotonic() < deadline:
        ...
        if lb_host:
            ...
            if http_ok(url, timeout_s=15.0):
                ...
                return PhaseResult("aws_validate", True, f"frontend at {url}", data={"frontend_url": url})
        time.sleep(15)
    return PhaseResult("aws_validate", False, f"frontend not reachable within {alb_timeout_s}s")
...
def print_summary(results: list[PhaseResult]) -> None:
    ...
    if all(r.ok for r in results):
        url = next((r.data.get("frontend_url") for r in results if r.data.get("frontend_url")), None)
        if url:
            log(f"\nFrontend URL: {url}")
