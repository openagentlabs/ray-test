"""Deploy phase functions — each returns ``DeployResult[PhaseOutcome]``."""
...
from deploy_to_aws.core.config import AppConfig
...
from deploy_to_aws.core.subprocess_runner import (
    run_command,
    run_command_checked,
    run_json_command,
)
from deploy_to_aws.core.types import DeployResult
from deploy_to_aws.deploy.models import PhaseOutcome
...
def terraform_var_file_args(config: AppConfig) -> DeployResult[list[str]]:
    args: list[str] = []
...
def phase_configure_aws(config: AppConfig) -> DeployResult[PhaseOutcome]:
    ...
    if "containers_eks_enabled" not in body:
        return Failure(
            AppError(
                ...
                detail=(
                    f"{tfvars.relative_to(config.repo_root)}: "
                    "set containers_eks_enabled = true"
                ),
            ),
        )
    if "containers_eks_enabled=true" not in compact:
        return Failure(
            AppError(
                ...
                detail=(
                    f"{tfvars.relative_to(config.repo_root)}: "
                    "containers_eks_enabled must be true"
                ),
            ),
        )
    ...
    return Success(
        PhaseOutcome(
            ...
            ok=True,
            detail=(
                f"APP_ENV={config.app.Env} APP_TARGET={config.app.Target} "
                f"account={account}"
            ),
        ),
    )
...
def phase_preflight(config: AppConfig) -> DeployResult[PhaseOutcome]:
    tools = {
...
def phase_validate_secrets(config: AppConfig) -> DeployResult[PhaseOutcome]:
    ...
    if not ecr_urls:
        return Success(
            PhaseOutcome(
                ...
                ok=False,
                detail=(
                    "containers_ecr_repository_urls empty — "
                    "enable containers_eks_enabled"
                ),
            ),
        )
...
def phase_build_images(
    config: AppConfig, *, no_cache: bool
) -> DeployResult[PhaseOutcome]:
    ...
    missing = [
        tag for tag in built_tags if docker_image_missing(tag, cwd=config.repo_root)
    ]
    if missing:
        return Success(
            PhaseOutcome(
    ...
    *,
    ...
    for workload_key, local_ref in LOCAL_IMAGES.items():
        ...
        if isinstance(tagged, Failure):
            return tagged
        pushed_result = run_command_checked(
            ["docker", "push", remote], cwd=config.repo_root
        )
...
def phase_helm_rollout(
    ...
    *,
    ...
    namespace = (
        outputs.get("containers_k8s_namespace", {}).get("value")
        or config.paths.K8sNamespace
    ).strip()
    ...
    if not chart_dir.is_dir():
        return Success(
            PhaseOutcome(
                ...
                ok=False,
                detail=(
                    f"missing Helm chart at {chart_dir.relative_to(config.repo_root)}"
                ),
            ),
        )
