...
from deploy_to_aws.core.config import AppConfig, load_app_config
...
from deploy_to_aws.deploy.models import DeployInvokeParams, DeploySummary, PhaseOutcome
from deploy_to_aws.deploy.phases import (
    default_image_tag,
    phase_aws_validate,
    phase_build_images,
    phase_configure_aws,
...
PhaseFn = Callable[[], DeployResult[PhaseOutcome]]


def merge_invoke_params(
    config: AppConfig, params: DeployInvokeParams
) -> DeployInvokeParams:
    ...
    return DeployInvokeParams(
        ...
        if params.skip_build is not None
        else config.deploy.SkipBuild,
        ...
        if params.image_tag is not None
        else config.deploy.ImageTag,
        no_cache=params.no_cache
        if params.no_cache is not None
        else config.deploy.NoCache,
        post_terraform_only=params.post_terraform_only,
    )


def run_phases(phases: list[tuple[str, PhaseFn]]) -> DeployResult[DeploySummary]:
...
def build_phase_list(
    ...
    *,
    ...
    def _terraform_phase() -> DeployResult[PhaseOutcome]:
                ...
                lambda: (
                    ...
                    if outputs
                        ...
                        PhaseOutcome(
                            ...
                            ok=False,
                            detail=(
                                "missing terraform outputs — run terraform phase first"
                            ),
                        ),
                    )
                ),
...
def execute_deploy(
    ...
    if merged.post_terraform_only:
        ...
        from deploy_to_aws.core.subprocess_runner import run_json_command
