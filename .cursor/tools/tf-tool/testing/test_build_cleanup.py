...
from tf_tool.build.cleanup import clean_project
...
def test_clean_project_removes_artifacts(tmp_path: Path) -> None:
    ...
    dist = tmp_path / "dist"
    dist.mkdir()
    (dist / "tf_tool-0.1.0.whl").write_text("wheel", encoding="utf-8")
    egg_info = tmp_path / "src" / "tf_tool.egg-info"
    egg_info.mkdir(parents=True)
    (egg_info / "PKG-INFO").write_text("info", encoding="utf-8")
    cache = tmp_path / "src" / "tf_tool" / "__pycache__"
