...
from tf_tool.core.env_check import (
...
def test_env_check_skipped_with_env_var(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("TF_TOOL_SKIP_ENV_CHECK", "1")
    result = run_env_check()
    assert isinstance(result, Success)
    assert result.unwrap().source == "TF_TOOL_SKIP_ENV_CHECK=1"
...
""".strip(),
    ...
    src = tmp_path / "src" / "tf_tool"
    src.mkdir(parents=True)
    monkeypatch.setattr("tf_tool.core.env_check.find_project_root", lambda: tmp_path)
    result = run_env_check()
    assert isinstance(result, Success)
    report = result.unwrap()
    assert "pyproject.toml" in report.source
    assert report.dependencies[0].name == "httpx"
