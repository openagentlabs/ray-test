"""Unit tests for installing tf-tool onto PATH."""
...
from tf_tool.build.constants import (
    CLI_SCRIPT_NAMES,
    OUTPUT_DIR,
    OUTPUT_DIST_DIR,
)
from tf_tool.build.install import install_cli
from tf_tool.build.package import build_package
...
def _fake_subprocess_run(command: list[str], cwd: Path) -> subprocess.CompletedProcess[str]:
    if command[:2] == ["uv", "build"]:
        ...
        wheel_path = dist_dir / "tf_tool-0.1.0-py3-none-any.whl"
        with zipfile.ZipFile(wheel_path, "w") as archive:
            archive.writestr("tf_tool/__init__.py", "__version__ = '0.1.0'\n")
            archive.writestr("tf_tool/cli.py", "def main() -> None: ...\n")
...
def test_install_cli_links_launchers(tmp_path: Path) -> None:
    ...
    import tf_tool.build.package as package_module
