...
from jp_tool.core.config import load_app_config
from jp_tool.core.results import Failure, Success
from jp_tool.core.types import TextResult, UnitResult
from jp_tool.core.validation import parse_invoke_model
from jp_tool.deploy.models import DeployInvokeParams
from jp_tool.deploy.pipeline import execute_deploy, format_summary
...
class DeployAction(ActionBase):
    ...
    def bind_cli(self, app: typer.Typer) -> UnitResult:
        ...
        def deploy_cmd(
            yes: bool | None = typer.Option(None, "--yes", help="Auto-approve terraform apply."),
            skip_build: bool | None = typer.Option(None, "--skip-build", help="Skip Docker build."),
...
class PostDeployAction(ActionBase):
    ...
    DESCRIPTION: ClassVar[str] = "Run build, ECR push, Helm rollout after terraform apply."
    VERSION: ClassVar[str] = "0.1.0"
