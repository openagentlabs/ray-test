...
class AppConfig:
    ...
    def __init__(self, root: AppConfigRoot, *, repo_root: Path, config_path: Path) -> None:
        self._root = root
...
def env_var_name(section: str, field: str) -> str:
    """Build the canonical UPPERCASE env key: ``JP_TOOL_<SECTION>_<FIELD>``."""
...
def apply_env_overrides(data: dict[str, Any]) -> dict[str, Any]:
    """Apply UPPERCASE ``JP_TOOL_*`` env vars on top of TOML (env wins)."""
...
def discover_repo_root(start: Path, configured: Option[str]) -> JpResult[Path]:
    ...
    return Failure(
        AppError(
            ...
            detail=(f"Started search at {start}. Set Paths.RepoRoot or JP_TOOL_PATHS_REPO_ROOT."),
        ),
    )
...
def load_app_config(
    *,
    ...
    Environment variables use the app prefix ``JP_TOOL_`` and UPPERCASE
    names ``JP_TOOL_<SECTION>_<FIELD>``. When set, they override TOML.
    """
