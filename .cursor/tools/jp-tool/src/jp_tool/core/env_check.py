"""Runtime Python and dependency validation for jp-tool."""
...
from jp_tool.build.gate import find_project_root
from jp_tool.core.errors import AppError, ErrorCodes
from jp_tool.core.types import JpResult

_SKIP_ENV: Final[str] = "JP_TOOL_SKIP_ENV_CHECK"
...
def _read_wheel_requirements() -> tuple[str, list[Requirement]] | None:
    try:
        dist = importlib.metadata.metadata("jp-tool")
...
def _load_requirements() -> tuple[str, list[Requirement], str]:
    ...
    if from_wheel is not None:
        ...
        return python_req, reqs, "installed package metadata (jp-tool)"
...
def run_env_check() -> JpResult[EnvCheckReport]:
    ...
    if os.environ.get(_SKIP_ENV) == "1":
        report = EnvCheckReport(
            ...
            dependencies=[],
            source="JP_TOOL_SKIP_ENV_CHECK=1",
            ok=True,
        )
