...
Skill path: **`.cursor/skills/aws_s3_create/`** (from repository root). The same **`SKILL.md`** and **`reference.md`** are mirrored under **`iam.service/.cursor/skills/aws_s3_create/`** and **`solutions.svc/.cursor/skills/aws_s3_create/`** — load **`SKILL.md`** when the user asks for guided S3 creation or names **`aws_s3_create`** / **`aws-s3-create`**.
...
3. **Deployed map** — using the resolved **`service`** from Phase A, ensure **`infra/deployed/aws/<AWS_ACCOUNT_ID>/<AWS_DEFAULT_REGION>/services/<service>/terraform/aws/s3/`** exists; add **`.gitkeep`** if the folder is new. Use **`AWS_ACCOUNT_ID`** and **`AWS_DEFAULT_REGION`** from **`.cursor/rules/constants.mdc`** (never guess another account or region).
...
- **Account / region / profile**: `.cursor/rules/infra.mdc` and **`.cursor/rules/constants.mdc`**
