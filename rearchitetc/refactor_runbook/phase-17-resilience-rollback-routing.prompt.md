# Strangler Fig Extraction — Phase 17 Prompt: Resilience, Rollback & Strangler Routing

> **Self-contained Cursor agent prompt** (execution-readiness set). Carries the full base CONTRACT and ADDENDA I–III; consumes the file produced by earlier phases, then enriches that single file. Requires §9 client and §18 runtime as input.
...
## PHASE 17 — Resilience, Rollback & Strangler Routing
**Run order:** 17 · **Inputs:** codebase **+** analysis file (§9, §16, §18 present) **+** ledger · **Enriches:** §19 Resilience, rollback & strangler routing

**Precondition:** §9 (strangler client) and §18 (runtime) exist. If absent, **HALT**.

**Objective:** design the production switch that routes between the in-process original and the externalized path, with failure handling and **instant revert** — the heart of a safe strangler migration.

**Actions**
1. **Routing toggle:** a feature-flag/switch that selects original vs externalized per call; default safe; runtime-changeable.
2. **Rollout strategy:** shadow (call both, serve original, compare), then canary (small %), then full — with the promotion/abort criteria per stage.
3. **Failure modes (FMEA):** externalized service down / slow / partial / divergent output — for each, the strangler client behaviour (fallback to original, fail-closed, retry/back-pressure) and blast-radius.
4. **Instant rollback:** the one-action revert to the in-process path and the conditions that trigger it.
5. **Diagrams:** routing/rollback as a `flowchart`; shadow-vs-canary-vs-full as a `stateDiagram-v2`.

**Gap pass (C11):** undefined abort criteria, missing fallback path, non-revertible side-effects — fill or register.

**Acceptance criteria**
- [ ] Routing toggle + staged rollout with promote/abort criteria.
- [ ] FMEA table with client behaviour per failure; instant-rollback path defined.

**Write-back:** §19 + ledger.
