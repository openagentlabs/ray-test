# Strangler Fig Extraction — Phase 10 Prompt: Test Strategy & Coverage Mapping

> **Self-contained Cursor agent prompt** (verification & testing set). Carries the full base CONTRACT, ADDENDUM I (scalable-compute), and ADDENDUM II (testing); consumes the file produced by earlier phases, then enriches that single file. Requires the §9 strangler interface contracts as input.
...
## PHASE 10 — Test Strategy & Coverage Mapping
**Run order:** 10 of 13 · **Inputs:** codebase **+** analysis file (§9 strangler contracts present; §11 if model modularization in scope) **+** ledger · **Enriches:** §12 Test strategy & coverage matrix

**Precondition:** §9 (strangler interface contracts) exists. If absent, name what is missing and **HALT** — there is nothing concrete to test yet.

**Objective:** enumerate **every testable unit (C19)**, apply the **test taxonomy (C18)**, and build the coverage matrix that proves each unit can be **fully verified individually**.

**Actions**
1. **Inventory testable units** from §3–§11: each compute unit, strangler client, interface (input + output), model-training module, and integrated stage path — with `path:line` evidence.
2. For each unit define **what must be verified** and its **oracle** (parity / golden dataset / property invariant / spec).
3. Build the **unit × test-type matrix** (C18): mark which test types apply and why.
4. Define **coverage targets** and **exit criteria** — the explicit proof that every component, client, and interface is testable in isolation (fakes/stubs for S3, DB, model libs, network).
5. Flag **untestable-as-is risks** — hidden global state, non-determinism, time/seed dependence — as gaps with the change needed to make them testable.

**Gap pass (C11):** missing oracles, un-isolatable dependencies, ambiguous interface contracts — fill or register.

**Acceptance criteria**
- [ ] Complete testable-unit inventory (C19), evidence-cited.
- [ ] Full unit × test-type matrix with oracles.
- [ ] Coverage targets + exit criteria + isolation strategy per unit.

**Write-back:** §12 + ledger. Synthetic data design is Phase 11.
