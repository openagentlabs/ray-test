# Strangler Fig Extraction — Phase 19 Prompt: Cut-Readiness Audit & Final Consistency Gate

> **Self-contained Cursor agent prompt** (execution-readiness set). Carries the full base CONTRACT and ADDENDA I–III; consumes the file produced by earlier phases, then enriches that single file. Requires §1–§20 and TEST-PLAN.md as input.
...
## PHASE 19 — Cut-Readiness Audit & Final Consistency Gate
**Run order:** 19 (terminal) · **Inputs:** codebase **+** the full analysis file + `TEST-PLAN.md` + ledger · **Enriches:** §21 Cut-readiness checklist + whole-document audit

**Precondition:** §1–§20 and `TEST-PLAN.md` exist. If any is missing, list them and **HALT**.

**Objective:** prove the entire plan is complete, internally consistent, and safe to act on — the single go/no-go gate (C26). Analysis only.

**Actions**
1. **Completeness audit:** every section §0–§20 present and populated; every stage/work-item in the ledger `COMPLETE` or explicitly `ACCEPTED`.
2. **Consistency audit:** cross-check for contradictions — a variable/interface/figure described differently in two places, an endpoint referenced but unspecified, a parity assertion missing a baseline. Resolve or register each.
3. **Gap closure:** confirm §7 holds every gap as CLOSED (evidence) or ACCEPTED (reason); **no silent gaps**. If any open gap remains, loop the owning phase until closed.
4. **Cut-readiness checklist (C26):** assemble §21 and mark each item green/red with the evidence link.
5. **Go/No-Go:** a single verdict with the blocking items if No-Go.
6. **Glossary:** consolidate key terms (C3, C15, C16, C19, C24–C26) into a definitions index for ARB readers.

**Acceptance criteria**
- [ ] Completeness + consistency audits done; all contradictions resolved/registered.
- [ ] §21 checklist complete; explicit Go/No-Go verdict; glossary added.

**On finish:** output the verdict, the path to both deliverables, the green/red checklist summary, and any blocking items. Then **stop** — do not continue past a No-Go without the named items closed.
