# Strangler Fig Extraction — Phase 3 Prompt: Gap Analysis & Remediation

> **Self-contained Cursor agent prompt.** It carries the full requirements (the CONTRACT below) **and** consumes the file produced by earlier phases, then enriches that single file. Requires the Phase 2 per-stage analysis as input.
...
**Run order:** 3 of 19 · **Inputs:** codebase + analysis file (§3 per-stage analysis present) + ledger · **Enriches:** §7 gap register (closure pass)

**Precondition:** §3 exists for all stages (03A–03I). If any stage is placeholder-thin, **HALT** and complete Phase 2 first.

**Objective:** second-pass gap closure — re-read code to close every registered gap; consolidate §7 document-wide register.

**Actions**
1. Collect all per-stage and bootstrap gaps from §0.5, §2, §3 gap registers.
2. For each **OPEN** gap: read code; update the owning section; mark **CLOSED** with evidence.
3. Gaps that cannot close → **ACCEPTED** with explicit reason in §7.
4. Reconcile resource-model assumptions (C10) across stages; fix contradictions.
5. Update ledger gap column for every affected work item.

**Gap pass (C11):** this phase *is* the gap pass — §7 must have zero silent gaps.

**Acceptance criteria**
- [ ] §7 lists every gap as CLOSED or ACCEPTED-with-reason.
- [ ] No contradiction between §3 03I and §6 consolidated resource model.
- [ ] Ledger gap columns updated.

**Write-back:** §7 + in-place fixes to §3/§6 where gaps closed. Then Phase 4.
