# Strangler Fig Extraction — Phase 9 Prompt: Model-Training Modularization

> **Self-contained Cursor agent prompt** (scalable-compute set). Carries the full base CONTRACT **and** the CONTRACT ADDENDUM, consumes the file produced by earlier phases, then enriches that single file. Requires the Phase 8 auto-training strangler client (section 10) as input.
...
## PHASE 9 — Model-Training Modularization
**Run order:** 9 of 9 · **Inputs:** codebase **+** analysis file (§10 auto-training strangler client present) **+** ledger · **Enriches:** §11 Model-training modularization plan

**Precondition:** §10 exists (auto-training strangler client defined). With the strangler client now standing in for auto-training, this phase decomposes what sits behind it. If §10 is absent, **HALT**.

**Objective:** define how to **separate each model training into its own independent module** behind the auto-training strangler client — analysis only.

**Actions**
1. **Enumerate** every model/algorithm trained inside auto-training, with `path:line` evidence.
2. **Per model:** separate **shared** code from **per-model** code; define the **module boundary**, its interface `train(inputs) -> outputs` (data + config in → artifact + metrics out), and its dependency closure.
3. **Common module interface + dispatch:** define one training-module contract and a **registry/dispatch** so each model is an independent, swappable module the strangler client (or externalized trainer) selects at runtime.
4. **Modularization order + risks:** sequencing, shared-state hazards, parity per module — analysis only.
5. **Diagrams:** module decomposition (`flowchart`) and the common per-module interface; show registry/dispatch routing.

**Gap pass (C11):** shared vs per-model boundaries, interface uniformity, per-model parity — fill or register.

**Acceptance criteria**
- [ ] Every model training enumerated and bounded as a module with a uniform interface.
- [ ] Common module contract + registry/dispatch defined.
- [ ] Modularization order, risks, and per-module parity captured.

**On finish:** output a one-paragraph summary, the path to `strangler-fig-analysis.md`, the dominating compute units at 1TB, and the recommended externalization + modularization order. Then **stop**.
