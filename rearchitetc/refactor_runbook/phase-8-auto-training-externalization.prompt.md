# Strangler Fig Extraction — Phase 8 Prompt: Auto-Training Externalization

> **Self-contained Cursor agent prompt** (scalable-compute set). Carries the full base CONTRACT **and** the CONTRACT ADDENDUM, consumes the file produced by earlier phases, then enriches that single file. Requires Phases 6-7 sections 8-9 as input.
...
## PHASE 8 — Auto-Training Externalization
**Run order:** 8 of 9 · **Inputs:** codebase **+** analysis file (§8–§9 present) **+** ledger · **Enriches:** §10 Auto-training externalization

**Precondition:** the auto-training stage/code is identified in §3/§8. If it cannot be located, **HALT** stating where you looked.

**Objective:** apply the Phase 6 + Phase 7 treatment specifically to the **auto-training code**, as one focused unit, with the same rigour and identical-interface guarantees.

**Actions**
1. **Locate & characterise** the auto-training code as a compute unit (C15): what it trains, algorithmic shape, passes over data, dependencies (data, feature inputs, model libs, config, artifact I/O).
2. **Hotness at 5M (C10)** + **scale model to rows@1TB (C14)**; identify what dominates training cost at scale.
3. **Cut boundary:** exact **input interface** (training data + config + hyperparameters) and exact **output interface** (model artifact + metrics), the enclosed training computation, and full dependency closure.
4. **Strangler client contract (C16):** identical I/O; dispatch to the externalized trainer; parity contract (same artifact/metrics for same input) + characterization tests; error/timeout/retry/idempotency semantics.
5. **Diagrams (before/after):** in-process auto-training vs `strangler-client → externalized trainer`, `flowchart` + `sequenceDiagram`.

**Gap pass (C11):** training inputs/outputs, artifact contract, non-determinism handling (seeds) for parity — fill or register.

**Acceptance criteria**
- [ ] Auto-training characterised with hotness@5M + scale@1TB.
- [ ] Cut boundary + identical-interface strangler client + parity/characterization tests complete.
- [ ] Before/after diagrams valid.

**Write-back:** §10 + ledger. Per-model module separation is Phase 9.
