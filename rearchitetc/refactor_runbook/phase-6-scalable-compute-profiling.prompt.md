# Strangler Fig Extraction — Phase 6 Prompt: Scalable-Compute Identification & Hotness Profiling

> **Self-contained Cursor agent prompt** (scalable-compute set). Carries the full base CONTRACT **and** the CONTRACT ADDENDUM, consumes the file produced by earlier phases, then enriches that single file. Requires Phase 4 seam maps as input.
...
## PHASE 6 — Scalable-Compute Identification & Hotness Profiling
**Run order:** 6 of 9 · **Inputs:** codebase **+** analysis file (Phase 4 seam maps present; Phase 5 ideally done) **+** ledger · **Enriches:** §8 Scalable-compute catalogue

**Precondition:** per-stage seam maps (§3 seam map / Phase 4) exist. If absent, name the stages and **HALT** (or run Phase 4 first). Compute units are profiled on top of confirmed seams, not over open gaps.

**Objective:** for **every stage**, identify each **compute unit (C15)** whose cost grows with row count, characterise it in detail, profile its hotness at 5M rows, and model its growth to the row count of a 1TB file (C14).

**Actions (per stage, breadcrumb order)**
1. Locate the compute block(s) on the data-heavy path — cross-reference the stage's 03H hot functions and 03F internal flow.
2. **Characterise each compute unit (C15):** what it computes, algorithmic shape, passes over the data, libraries, and full dependency list (incl. hidden global/shared state).
3. **Hotness at 5M (C10):** CPU class + peak memory; mark the **hottest unit** in the stage and say why (vectorised vs row-wise vs Python-loop, copies, derived frames).
4. **Scale model (C14):** project CPU + peak memory at 5M / 25M / 100M / rows@1TB; assign a growth class; name the unit that **dominates at 1TB**.
5. **Diagrams:** mark the compute units on the stage L3 call graph; add the C14 scale table.

**Gap pass (C11):** resource inputs, algorithmic shape, dependency closure — fill from code or register.

**Acceptance criteria (per stage)**
- [ ] Every scaling compute unit catalogued with C15 fields, evidence-cited.
- [ ] Hotness at 5M with arithmetic; hottest unit identified.
- [ ] C14 scale table to rows@1TB with growth class and the 1TB-dominating unit.

**Write-back:** §8 per stage + ledger. Boundary/interface design is Phase 7.
