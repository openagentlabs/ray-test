# Strangler Fig Extraction — Phase 11 Prompt: Synthetic Data & Fixtures Design

> **Self-contained Cursor agent prompt** (verification & testing set). Carries the full base CONTRACT, ADDENDUM I (scalable-compute), and ADDENDUM II (testing); consumes the file produced by earlier phases, then enriches that single file. Requires the Phase 10 coverage matrix (§12) as input.
...
## PHASE 11 — Synthetic Data & Fixtures Design
**Run order:** 11 of 13 · **Inputs:** codebase **+** analysis file (§12 present) **+** ledger · **Enriches:** §13 Synthetic data & fixtures

**Precondition:** §12 coverage matrix exists. If absent, **HALT**.

**Objective:** design the **synthetic data (C20)** that drives every test type for every unit, at every required scale, with deterministic, safe, schema-faithful generation.

**Actions**
1. **Reverse the schema** of the credit-rating CSV from code/§3: columns, types, ranges, distributions, inter-column relationships; flag unknowns as gaps.
2. **Define a seeded generator** producing: small unit fixtures, **edge/adversarial** fixtures (nulls, out-of-range, empty, duplicate, malformed, encoding), and **scale fixtures** at 5M / 25M / 100M / rows@1TB-equivalent (tie to the C14 byte basis).
3. **Golden datasets** — specify how original boundary I/O pairs are captured to drive **parity** tests (C16).
4. **Fixture → test mapping** — which fixture set each testable unit and test type consumes.
5. **Provenance & safety** — synthetic only, no real PII; document generation determinism (seed → dataset).

**Gap pass (C11):** schema ranges/distributions, scale-fixture feasibility, golden-capture method — fill or register.

**Acceptance criteria**
- [ ] Schema-faithful generator spec (small + edge + scale fixtures), deterministic & seeded.
- [ ] Golden-dataset capture method for parity defined.
- [ ] Fixture-to-test/unit mapping complete; safety documented.

**Write-back:** §13 + ledger. Concrete per-artifact cases are Phase 12.
