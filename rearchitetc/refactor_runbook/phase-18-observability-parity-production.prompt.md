# Strangler Fig Extraction — Phase 18 Prompt: Observability & Parity-in-Production

> **Self-contained Cursor agent prompt** (execution-readiness set). Carries the full base CONTRACT and ADDENDA I–III; consumes the file produced by earlier phases, then enriches that single file. Requires §16 baseline and §17 routing as input.
...
## PHASE 18 — Observability & Parity-in-Production
**Run order:** 18 · **Inputs:** codebase **+** analysis file (§16, §17 present) **+** ledger · **Enriches:** §20 Observability & parity-in-production

**Precondition:** §16 (baseline capture) and §17 (routing) exist. If absent, **HALT**.

**Objective:** design the telemetry and **live shadow-comparison** that prove parity on real traffic and detect divergence the moment it appears.

**Actions**
1. **Metrics:** latency, throughput, error rate, resource use for the externalized path vs original; tie thresholds to the C14 budget.
2. **Shadow comparison:** during the shadow stage (§17), compare `externalized(input)` to `original(input)` on live inputs using the C25 parity assertion; record divergence rate.
3. **Logging & tracing:** correlation IDs across strangler client → externalized compute; what to log (never raw PII — see §17).
4. **Alerting:** divergence, latency/error budget breach, scale-threshold breach — with the action each triggers (incl. auto-rollback link to §19).
5. **Dashboards:** the views that show go/no-go at a glance.

**Gap pass (C11):** unmeasurable divergence, PII-in-logs risk, missing alert actions — fill or register.

**Acceptance criteria**
- [ ] Metrics + live parity-comparison + tracing + alerting (with actions) defined.
- [ ] PII-safe logging confirmed; dashboards specified.

**Write-back:** §20 + ledger.
