# Strangler Fig Extraction — Phase 16 Prompt: Externalized Runtime Topology & Deployment

> **Self-contained Cursor agent prompt** (execution-readiness set). Carries the full base CONTRACT and ADDENDA I–III; consumes the file produced by earlier phases, then enriches that single file. Requires §8 scale and §9 contracts as input.
...
## PHASE 16 — Externalized Runtime Topology & Deployment
**Run order:** 16 · **Inputs:** codebase **+** analysis file (§8 scale, §9 contracts, §15 security present) **+** ledger · **Enriches:** §18 Externalized runtime topology & deployment

**Precondition:** §8 (scale model) and §9 (contracts) exist. If absent, **HALT**.

**Objective:** replace "runs externally" with a concrete, scale-appropriate runtime for the externalized compute and training — design only.

**Actions**
1. **Packaging:** turn each compute unit's dependency closure (§9) into a self-contained deployable artifact; pin runtime + libs.
2. **Topology options:** evaluate runtime forms (e.g. container service, serverless, distributed/parallel executor) against the 5M→rows@1TB scale curve (§8/C14); recommend one per unit with rationale.
3. **Provisioning & scaling:** resource sizing tied to the C14 projections; horizontal/vertical scaling and concurrency model; cold/warm behaviour.
4. **Cost note:** order-of-magnitude cost at 5M and at rows@1TB for the recommended topology; flag the dominant cost driver.
5. **Diagrams:** deployment topology (`flowchart`) showing strangler client → externalized runtime → data stores.

**Gap pass (C11):** undefined scaling limits, packaging blockers, missing provisioning inputs — fill or register.

**Acceptance criteria**
- [ ] Deployable packaging + recommended topology per unit, justified against the scale curve.
- [ ] Provisioning/scaling model + cost note + deployment diagram.

**Write-back:** §18 + ledger.
