# Strangler Fig Extraction — Phase 12 Prompt: Component, Client & Interface Test Design

> **Self-contained Cursor agent prompt** (verification & testing set). Carries the full base CONTRACT, ADDENDUM I (scalable-compute), and ADDENDUM II (testing); consumes the file produced by earlier phases, then enriches that single file. Requires §12–§13 as input.
...
## PHASE 12 — Component, Client & Interface Test Design
**Run order:** 12 of 13 · **Inputs:** codebase **+** analysis file (§12–§13 present) **+** ledger · **Enriches:** §14 Per-artifact test designs

**Precondition:** §12 (matrix) and §13 (fixtures) exist. If absent, **HALT**.

**Objective:** design **concrete test cases** for each extracted **component**, **strangler client**, and **interface** — with explicit input/output parameters, expected results, and the fixture/oracle each uses.

**Actions (per testable unit)**
1. **Unit tests** — function-level: input parameters → expected output, using §13 fixtures.
2. **Interface / contract tests** — validate the **input** interface (schema, types, ordering, invariants, null/edge handling) and the **output** interface (schema, types, invariants).
3. **Parity / characterization** — assert `strangler_client(input) == original(input)` and `externalized(input) == in_process(input)` over golden datasets; the harness that proves drop-in equivalence.
4. **Negative / edge** — the adversarial fixtures, with the expected failure/handling for each.
5. **Property-based** — invariants that must hold across generated inputs.
6. **Isolation harness** — the fakes/stubs for S3, DB, model libs, network so each unit runs standalone (design only — no implementation).

**Gap pass (C11):** missing expected outputs, undefined error semantics, parity non-determinism (seeds) — fill or register.

**Acceptance criteria (per unit)**
- [ ] Unit, contract, parity, negative/edge, and property cases designed with input/output params + oracle.
- [ ] Isolation harness specified so the unit, client, and interface are each testable individually.

**Write-back:** §14 + ledger. Integration, scale and the test document are Phase 13.
