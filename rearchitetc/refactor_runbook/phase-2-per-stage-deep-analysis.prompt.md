# Strangler Fig Extraction — Phase 2 Prompt: Per-Stage Deep Analysis

> **Self-contained Cursor agent prompt.** It carries the full requirements (the CONTRACT below) **and** consumes the file produced by earlier phases, then enriches that single file. Requires the Phase 1 stage index as input.
...
## PHASE 2 — Per-Stage Deep Analysis
**Run order:** 2 of 19 · **Inputs:** codebase + analysis file (§2 stage index present) + ledger · **Enriches:** §3 (03A–03I per stage)

**Precondition:** §2 stage index exists. If absent, **HALT** — run Phase 1 first.

**Objective:** for **every breadcrumb stage**, complete sub-steps **03A–03I** (C7) with evidence-cited claims.

**Actions (per stage, breadcrumb order)**
1. **03A** Purpose & UX — components, roles, input/display types.
2. **03B** Data provenance — user-entered / user-selected / derived classification.
3. **03C** State & storage — full C8 variable table (FE + BE).
4. **03D** Order of operations — FE → API → BE → response → FE.
5. **03E** API mapping — method, path, shapes, FE call sites.
6. **03F** Backend mapping — internal state flow per endpoint.
7. **03G** Diagrams L0–L3 (C9) — all four zoom levels as Mermaid.
8. **03H** Variable inventory & hot functions on the data-heavy path.
9. **03I** Resource model @ 5M rows (C10) with arithmetic table.
10. Per-stage gap register (CLOSED or ACCEPTED).

**Gap pass (C11):** fill from code or register; no silent gaps.

**Acceptance criteria (per stage)**
- [ ] All 03A–03I present and evidence-cited.
- [ ] C8 variable table and L0–L3 diagrams valid Mermaid.
- [ ] 03I shows assumptions and peak memory arithmetic.

**Write-back:** §3 per stage + ledger. Seam maps are Phase 4; do not skip thin stages.
