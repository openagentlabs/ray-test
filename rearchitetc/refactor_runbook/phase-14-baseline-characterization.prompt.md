# Strangler Fig Extraction — Phase 14 Prompt: Baseline Characterization Capture

> **Self-contained Cursor agent prompt** (execution-readiness set). Carries the full base CONTRACT and ADDENDA I–III; consumes the file produced by earlier phases, then enriches that single file. Requires §9 contracts and §13 fixtures as input.
...
## PHASE 14 — Baseline Characterization Capture (design)
**Run order:** 14 · **Inputs:** codebase **+** analysis file (§9 contracts, §13 fixtures present) **+** ledger · **Enriches:** §16 Baseline characterization capture

**Precondition:** §9 (interface contracts) and §13 (fixtures) exist. If absent, **HALT**. Parity needs both a boundary and inputs.

**Objective:** design the procedure that records **current in-process behaviour** at every extraction boundary as the **parity ground truth (C25)** — so `externalized == original` is provable. **Design only; execution is a separate gated step (see runbook).**

**Actions**
1. **Capture points:** for each compute unit / strangler client / training boundary (§9, §10), define exactly what input and output to record.
2. **Capture method:** how to instrument the boundary read-only to log golden input/output pairs without altering behaviour; where captures are stored.
3. **Determinism controls (C25):** fix seeds; identify non-deterministic ops; define the parity assertion (exact / tolerance / canonicalization) per output.
4. **Coverage:** map golden captures to the §13 fixture sets (normal, edge, scale) so parity covers the input space.
5. **Versioning:** stamp each baseline with the interface version (C24) it characterizes.

**Gap pass (C11):** uncapturable outputs, hidden side-effects, non-determinism without an assertion strategy — fill or register.

**Acceptance criteria**
- [ ] Capture points, method, storage, and determinism controls defined for every boundary.
- [ ] Parity assertion strategy per output; baselines version-stamped.

**Write-back:** §16 + ledger.
