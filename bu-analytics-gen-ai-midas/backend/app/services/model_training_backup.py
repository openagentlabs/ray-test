...
class ModelTrainingService:
    ...
    async def run_auto_training(self, dataset_id: str, target_column: str, 
        ...
        """Main method to run the complete auto training pipeline with sampling and segmentation support"""
        ...
        try:
            ...
            # Step 1.5: Apply pipeline configuration if provided
            if pipeline_config:
                from app.services.pipeline_config_service import pipeline_config_service
                pipeline_result = pipeline_config_service.apply_pipeline_config(df, pipeline_config)
                
                # Use the train data from pipeline result
                train_data = pipeline_result['sampling']['train_data']
                test_data = pipeline_result['sampling']['test_data']
                
                self.logger.info(f"Applied pipeline config: {pipeline_result['pipeline_summary']}")
                self.add_training_log(model_id, f"Applied pipeline config: {pipeline_result['pipeline_summary']}")
                
                # For training, we use the train data
                df_for_training = train_data
            else:
                df_for_training = df
                test_data = None
            ...
            X, y = self.preprocess_data(df_for_training, target_column, independent_variables)
