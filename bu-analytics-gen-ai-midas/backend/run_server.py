#!/usr/bin/env python3
"""
Startup script for MIDAS FastAPI Backend using virtual environment
"""
...
def main():
    ...
    if not hasattr(sys, 'real_prefix') and not (hasattr(sys, 'base_prefix') and sys.base_prefix != sys.prefix):
        print("Warning: Not running in virtual environment. Starting with venv Python...")

        # Use virtual environment's Python
        venv_python = Path("./venv/Scripts/python.exe")
        if venv_python.exists():
            print(f"Using virtual environment: {venv_python}")
            os.execv(str(venv_python), [str(venv_python), "-m", "uvicorn", "main:app", "--reload", "--host", "0.0.0.0", "--port", "8000"])
        else:
